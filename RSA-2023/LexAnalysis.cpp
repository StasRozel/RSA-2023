...
namespace Lexer
{
		...
		bool& lex_ok)			// флаг ошибки(по ссылке)
	{
		...
		{
			Log::writeError(log.stream, Error::GetError(304, line, 0));		// в объявлении отсутствует ключевое слово variable
			Log::writeError(&std::cout, Error::GetError(304, line, 0));		// в объявлении отсутствует ключевое слово variable
			lex_ok = false;
		}
		if (i > 1 && itentry->idtype == IT::IDTYPE::F && tables.lextable.table[i - 1].lexema != LEX_FUNCTION)
		{
			...
			Log::writeError(&std::cout, Error::GetError(303, line, 0));		// в объявлении не указан тип функции
			lex_ok = false;
		}
